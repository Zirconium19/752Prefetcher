...
for i, x_cat in enumerate(x_categories):
    ...
    # 遍历所有分组类别，收集有效数据（包括0值）
    for j, category in enumerate(group_categories):
        value = trace_data[category]
        if not pd.isna(value):  # 只过滤NaN值，保留0值
            valid_categories.append(category)
            valid_values.append(value)
            color = color_mapping.get(category, custom_colors[j % len(custom_colors)])
            valid_colors.append(color)
            label = label_mapping.get(category, category)
            valid_labels.append(label)
    
    # 计算当前trace的位置
    x_pos = i * (len(group_categories) * (bar_width + bar_spacing) + group_spacing)
...
for i, category in enumerate(ordered_categories):
    label = label_mapping.get(category, category)
    color = color_mapping.get(category, custom_colors[i % len(custom_colors)])
